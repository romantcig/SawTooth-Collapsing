...
// ---- 保留兼容：applyPhaseDecay / forceStubAllTools / truncateTextBlocksDecay / minInt ----

// applyPhaseDecay 按指定阈值重新截断文本并重新桩化工具。
// userMax/userTruncated 为用户消息的截断阈值，assistant 阈值为其一半。
func applyPhaseDecay(messages []Message, tc *TokenCounter, userMax, userTruncated int) []Message {
	result := make([]Message, 0, len(messages))
	for _, msg := range messages {
		blocks, isArray := parseContent(msg.Content)

		blocks = truncateTextBlocksDecay(blocks, msg.Role, userMax, userTruncated)
		blocks = stubToolResults(blocks, nil) // Phase D: nil map → no deep_search hints in legacy path
...
// truncateTextBlocksDecay 按衰减阶段的自定义阈值截断文本块。
func truncateTextBlocksDecay(blocks []ContentBlock, role string, userMax, userTruncated int) []ContentBlock {
	var maxRunes, truncatedRunes int
	switch role {
	case "user":
		maxRunes = userMax
		truncatedRunes = userTruncated
	case "assistant":
		maxRunes = userMax / 2
		truncatedRunes = userTruncated / 2
	default:
		return blocks
	}
...
// forceStubAllTools 强制将所有 tool_use 和 tool_result block 桩化。
// 用于 DecayCompacted 阶段的无条件压缩。
// Phase D: 追加 deep_search 提示到桩化文本中。
func forceStubAllTools(messages []Message) []Message {
	// Phase D: 构建 tool_use_id → 关键词映射
		...
		for i, block := range blocks {
			if block.Type == "tool_result" {
				...
				if kw, ok := toolKWMap[block.ToolUseID]; ok && kw != "" {
					stub += " → deep_search('" + sanitizeDSKeyword(kw) + "')"
				}
				blocks[i] = ContentBlock{Type: "text", Text: stub}
				changed = true
			} else if block.Type == "tool_use" {
				annotation := extractAnnotation(block.Input)
				var text string
				if annotation != "" {
					text = "[→] " + block.Name + " " + formatArgsSummary(block.Input) + " — " + annotation
				} else {
					text = "[→] " + block.Name + " " + formatArgsSummary(block.Input)
				}
				// Phase D: 追加 deep_search 提示
				if kw := extractToolKeywords(block); kw != "" {
					text += " → deep_search('" + sanitizeDSKeyword(kw) + "')"
				}
